...
url = 'https://rincondelmotor.com/add/'
url = 'https://bostonmais.com/adverts/add/'
url = 'https://saldo-doska.co.il/adverts-2/add/'
url = 'https://xaymacainternational.com/adverts/add/'
url = 'https://diariodelhuila.com/clasificados/add/'
...
def EventPost(contact_person,email,phone,title,description,price,location):
    driver.get(url)
    WebDriverWait(driver, 20).until(EC.element_to_be_clickable((By.NAME, 'submit')))
    ...
    try:
        ...
        pass
    try:
        _description = driver.execute_script(f'tinyMCE.activeEditor.setContent(`{description}`)')
    except:
        pass
    
    try:
        _description = driver.execute_script(f'(`document.getElementById("post_content").value={description}`)')
    except:
        traceback.print_exc()
    ...
    WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.CLASS_NAME,"adverts-cancel-unload")))
    try:
        _publish = driver.find_elements_by_class_name("adverts-cancel-unload")[1].click()
    except:
        pass
    try:
        _publish = driver.find_elements_by_class_name("adverts-cancel-unload")[0].click()
    except:
        pass
    
    
    driver.implicitly_wait(2)
    title_text = str(_title)
    # event_link = driver.find_element_by_xpath(f"//a[contains(text(),{title_text})]").click()
    # event_link = driver.find_element_by_xpath("//p//a").click()
    
    WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.XPATH,"//p//a")))
    event_link = driver.find_elements_by_xpath("//p//a")[0].click()
    print(event_link)
