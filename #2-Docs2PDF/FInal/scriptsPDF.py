...
second_heading_color=RGBColor(255, 255, 255)

logo_path='G:\ProjectWorkHere\Projects_MK\#2-Docs2PDF\FInal\images\logo.png'
...
def add_play_button(back,button):
    ...
    button = button.resize((300, 200), Image.ANTIALIAS)
...
for post_no in range(1, len(PostInfo) + 1):
    ...
    for images in range(1, len(PostInfo[post_no]['image_location']) + 1):
        doc = Document()
        if len(PostInfo[post_no]['description'])==0:
            newdescription = make_description(description_text,post_no,TeamA,TeamB)
        else:
            newdescription = make_description(PostInfo[post_no]['description'],post_no,TeamA,TeamB)
        ...
        try:
            add_play_button(image_link,logo_path)
